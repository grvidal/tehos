...
router.get('/cart', function(req, res, next) 
{
    ...
    {
        let query = "SELECT product_id, productname, productimage, description, weight, length, width, price, status, packaging_id, category_id FROM product WHERE product_id IN (" + req.session.cart + ") order by find_in_set(product_id, '" + req.session.cart + "');";
...
router.get('/checkout', function(req, res, next) 
{
    ...
    if (typeof req.session.customer_id !== 'undefined' && req.session.customer_id ) 
    {
        ...
        let insertquery = "INSERT INTO saleorder(customer_id, saledate, paymentstatus, authorizationnum) VALUES (?, now(), 'Paid', '12345678')"; db.query(insertquery,[req.session.customer_id],(err, result) => 
        {
            if (err) 
            { 
                console.log(err); 
            ...
            else 
            {
                ...
                req.session.cart.forEach((cartitem, index) => 
                {
                    ...
                    let insertquery = "INSERT INTO orderdetail(order_id, product_id, price, qty) VALUES (?, ?, (SELECT price from product where product_id = " + cartitem + "), ?)";
